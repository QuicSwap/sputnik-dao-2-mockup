...
export const Proposal = (props) => {
  ...
  const [gasAmount, setGasAmount] = useState(stateCtx.config.gasOptions);
  ...
  const handleGasChange = (e) => {
    setGasAmount({ ...gasAmount, value: toTeraGas(e.target.value).toFixed()});
  }

  const decrease = () => {
    setGasAmount({ ...gasAmount , value: toTeraGas(fromTeraGas(gasAmount.value).minus(gasAmount.step)).toFixed()});
  }

  const increase = () => {
    setGasAmount({ ...gasAmount , value: toTeraGas(fromTeraGas(gasAmount.value).plus(gasAmount.step)).toFixed()});
  }

  const handleVoteYes = () => {
    if (
      props.data.votes[window.walletConnection.getAccountId()] === undefined
    ) {
      vote("VoteApprove")
        .then()
        .catch((e) => {
        ...
        <MDBCol className="col-12 col-sm-8 col-lg-6 mx-auto">
          ...
          <MDBCard className="mb-5 stylish-color white-text">
            <MDBCardHeader className="text-center h4-responsive">
              ...
              props.data.kind.AddMemberToRole.role === "council"
                ? "Add " +
                  props.data.kind.AddMemberToRole.member_id +
                  " to the council"
                : null}
              {props.data.kind.RemoveMemberFromRole &&
              props.data.kind.RemoveMemberFromRole.role === "council"
                ? "Remove " +
                  props.data.kind.RemoveMemberFromRole.member_id +
                  " from the council"
                : null}
              ...
              props.data.kind.Transfer.token_id === ""
                ? "Request for payout " +
                  "Ⓝ" +
                  (props.data.kind.Transfer.amount / yoktoNear)
                    .toFixed(2)
                    .toString()
                    .replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
                  " to " +
                  props.data.kind.Transfer.receiver_id
                : null}
              {metadata &&
              props.data.kind.Transfer &&
              props.data.kind.Transfer.token_id !== ""
                ? "Request for payout " +
                  props.data.kind.Transfer.token_id
                    .split(".")[0]
                    .toUpperCase() +
                  new Decimal(props.data.kind.Transfer.amount)
                    .div("1e" + metadata)
                    .toFixed(2)
                    .toString()
                    .replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
              ...
              props.data.kind.FunctionCall.actions[0].method_name ===
                "create_token"
                ? "Create token"
                : null}
              {props.data.kind === "UpgradeSelf"
                ? "UpgradeSelf: " + props.data.target
                : null}
              {props.data.kind === "UpgradeRemote"
                ? "UpgradeRemote: " + props.data.target
                : null}
              {props.data.kind === "Transfer"
                ? "Transfer: " + props.data.target
                : null}
              {props.data.kind === "SetStakingContract"
                ? "SetStakingContract: " + props.data.target
                : null}
              {props.data.kind === "AddBounty"
                ? "AddBounty: " + props.data.target
                : null}
              {props.data.kind === "BountyDone"
                ? "BountyDone: " + props.data.target
                : null}
          ...
          */}
            ...
            <MDBCardBody className="white-text">
              <div className="float-left">
                {jsonError ? (
                  <MDBAlert color="danger" className="font-small text-center">
                    This proposal is created with an incorrect data, please
                    consider removing and creating a new one
                  </MDBAlert>
                ) : null}
                {props.data.kind.AddMemberToRole ||
                props.data.kind.RemoveMemberFromRole ? (
              ...
              <MDBCardText>
                ...
                {!jsonError &&
                props.data.kind.FunctionCall &&
                props.data.kind.FunctionCall.actions[0] &&
                props.data.kind.FunctionCall.actions[0].method_name ===
                  "create_token" &&
                JSON.parse(atob(props.data.kind.FunctionCall.actions[0].args))
                  .args ? (
                  <MDBBox
                    className="float-right h4-responsive"
                    style={{ width: "50%" }}
                  >
                    <a
                      className="text-right float-right white-text btn-link"
                      target="_blank"
                      style={{ wordBreak: "break-word" }}
                      href={
                        stateCtx.config.network.explorerUrl +
                        "/accounts/" +
                        props.data.kind.FunctionCall.receiver_id.toLowerCase()
                      }
                    >
                      {props.data.kind.FunctionCall.receiver_id.toLowerCase()}
                    </a>
                  ...
                  >
                    <a
                      ...
                      href={
                        stateCtx.config.network.explorerUrl +
                        "/accounts/" +
                        props.data.proposer.toLowerCase()
                      }
                    >
                      {props.data.proposer.toLowerCase()}
                    </a>
                ...
                props.data.kind.FunctionCall.actions[0].method_name !==
                  "create_token" &&
                props.data.kind.FunctionCall.actions[0] &&
                atob(props.data.kind.FunctionCall.actions[0].args) ? (
                  <>
                    <MDBBox
                      className="float-right h4-responsive"
                      style={{ width: "80%" }}
                    >
                      <a
                        ...
                        href={
                          stateCtx.config.network.explorerUrl +
                          "/accounts/" +
                          props.data.kind.FunctionCall.receiver_id
                        }
                      >
                        {props.data.kind.FunctionCall.receiver_id}
                      </a>
                ...
                JSON.parse(atob(props.data.kind.FunctionCall.actions[0].args))
                  ...
                  <>
                    ...
                    <MDBBox
                      className="float-left h5-responsive white-text"
                      style={{ width: "80%" }}
                    >
                      total supply:{" "}
                      {new Decimal(
                ...
                */}
                  ...
                  <>
                    ...
                    <MDBBox className="float-right h4-responsive white-text">
                      {props.data.kind.Transfer.token_id === "" ? (
                        ...
                        <>
                          {metadata && props.data.kind.Transfer.token_id
                            ? props.data.kind.Transfer.token_id +
                              " " +
                              new Decimal(props.data.kind.Transfer.amount)
                                .div("1e" + metadata)
                ...
                atob(props.data.kind.FunctionCall.actions[0].args) ? (
                  <>
                    ...
                    <MDBBox className="float-right h4-responsive white-text">
                      {new Decimal(
                        props.data.kind.FunctionCall.actions[0].deposit
                      )
                        .div(yoktoNear)
                        .toString()}
                    </MDBBox>
            ...
            <div className="rounded-bottom mdb-color text-center pt-3 pl-5 pr-5">
              <ul className="list-unstyled list-inline font-small">
                ...
                <li className="list-inline-item pr-2">
                  <div>
                    {props.data.votes !== undefined &&
                    Object.keys(props.data.votes).length !== 0 &&
                    Object.values(props.data.votes).includes("Approve") ? (
                      <MDBPopover
                        placement="top"
                        popover
                        clickable
                      ...
                      >
                        <div className="d-inline-block">
                          <MDBIcon
                            icon="thumbs-up"
                            size="2x"
                            className="lime-text mr-1"
                          />
                        </div>
                        <div>
                          <MDBPopoverBody>
                            <div className="h4-responsive">
                              {Object.keys(props.data.votes).map(
                                (item, key) => (
                                  <>
                ...
                <li className="list-inline-item pr-2">
                  <div>
                    {props.data.votes !== undefined &&
                    Object.keys(props.data.votes).length !== 0 &&
                    Object.values(props.data.votes).includes("Remove") ? (
                      <MDBPopover
                        placement="top"
                        popover
                        clickable
                ...
                <li className="list-inline-item pr-2">
                  <div>
                    {props.data.votes !== undefined &&
                    Object.keys(props.data.votes).length !== 0 &&
                    Object.values(props.data.votes).includes("Reject") ? (
                      <MDBPopover
                        placement="top"
                        popover
                        clickable
...
const ProposalPage = () => {
  ...
  useEffect(() => {
    window.contract
      .get_proposals({ from_index: parseInt(proposal), limit: 1 })
      .then((list) => {
        const t = [];
        list.map((item, key) => {
          const t2 = {};
          ...
          <MDBContainer style={{ minHeight: "100vh" }} className="mt-5">
            <MDBCol className="col-12 col-sm-8 col-lg-6 mx-auto mb-3">
              <MDBCard className="stylish-color-dark">
                <MDBCardBody className="text-left p-4 m-4 white-text">
                  ...
                  <MDBLink
                    to={"/" + dao}
                    className="elegant-color white-text text-center"
                  >
                    BACK TO DAO
                  </MDBLink>
                </MDBCardBody>
            ...
            {proposals !== null
              ? proposals.map((item, key) => (
                  <Proposal
                    data={item}
                    key={parseInt(proposal)}
                    id={parseInt(proposal)}
