...
const Dao = () => {
  ...
  const changeHandler = (event) => {
    if (event.target.name === "proposalTarget") {
     ...
     if (event.target.name === "gasAmount") {
      setGasAmount({
        ...gasAmount,
        value: toTeraGas(event.target.value).toFixed(),
      });
    }
  ...
  const submitProposal = async (e) => {
    ...
    if (e.target.name === "removeProposalCouncilMember") {
      const councilAccountValid = daoPolicy.roles[1].kind.Group.includes(
  ...
  const GasInput = (
      <MDBContainer className="m-0 p-0">
        <MDBInputGroup
            ...
            prepend={
              <span className="input-group-text text-muted md-addon">
                gas
              </span>
            }
            append={
                      ...
                      <MDBCol className="col-12 col-md-6 col-lg-5 mb-1">
                        <MDBCard className="p-md-3 m-md-3 stylish-color-dark">
                          <MDBCardBody className="text-center white-text">
                            ...
                            <MDBBtn
                              disabled={
                                daoPolicy.roles[1].kind.Group.length < 2
                              }
