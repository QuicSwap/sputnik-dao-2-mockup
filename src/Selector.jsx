...
const NewDao = (props) => {
  ...
  const validateName = (field, name, showMessage) => {
    const allowedChars =
      /^(?=[0-9a-zA-Z])(?=.*[0-9a-zA-Z]$)(?!.*__.*)(?!.*--.*)[0-9a-zA-Z_\-]*$/;
    if (
      name &&
      name.length >= 2 &&
      name.length <= 35 &&
      allowedChars.test(name)
    ) {
      return true;
    } else {
      showMessage(
                ...
                <MDBCardBody className="text-left">
                  ...
                  <div className="float-right">
                    {daoPolicy && daoPolicy.bounty_bond !== null
                      ? new Decimal(daoPolicy.bounty_bond.toString())
                          .div(yoktoNear)
                          .toString()
                      : ""}
                  </div>
            ...
            {daoExists ? (
              <>
                <div className="float-left">
                  <MDBBtn color="elegant" size="sm" onClick={toggleCollapse}>
                    council{" "}
                    <MDBIcon
                      icon={!collapseState ? "arrow-down" : "arrow-up"}
                    />
                  </MDBBtn>
                </div>
                <div className="float-right">
